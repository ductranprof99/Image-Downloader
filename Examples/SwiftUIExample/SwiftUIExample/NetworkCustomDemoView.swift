...
class NetworkCustomViewModel: ObservableObject {
    ...
    private var customConfig: ImageDownloaderConfigProtocol?
    ...
    func updateNetworkConfig() {
        ...
        let retryPolicy = RetryPolicy(
            ...
            enableLogging: enableRetryLogging
        )

        // Build config
        var networkConfig = DefaultNetworkConfig()
        networkConfig.maxConcurrentDownloads = maxConcurrent
        networkConfig.timeout = timeout
        networkConfig.allowsCellularAccess = allowsCellular
        networkConfig.enableBackgroundTasks = enableBackgroundTasks
        networkConfig.retryPolicy = retryPolicy
        networkConfig.customHeaders = headers

        customConfig = DefaultConfig(networkConfig: networkConfig)
    }
